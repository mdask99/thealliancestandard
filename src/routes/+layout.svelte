...
<div
	class={agent
		? 'min-h-dvh bg-terminal font-mono text-neutral-200 antialiased transition-colors duration-300'
		: 'min-h-dvh bg-paper font-sans text-ink antialiased transition-colors duration-300'}
>
	<header
		class={agent
			? 'sticky top-0 z-20 border-b border-gridline bg-terminal/90 backdrop-blur-md'
	...
	>
		...
		>
			<a
				href="/"
				class={agent
					? 'text-[11px] font-medium tracking-[0.22em] text-phosphor uppercase sm:text-sm'
			...
			>
				<span class={agent ? 'text-neutral-500' : 'text-ink'}>Human</span>
				<span
					class={agent
						? 'relative h-6 w-11 rounded-full border border-phosphor-dim bg-scan'
						: 'relative h-6 w-11 rounded-full border border-rule bg-white'}
				>
					<span
						class={agent
							? 'absolute top-0.5 left-0.5 h-5 w-5 translate-x-5 rounded-full bg-phosphor'
							: 'absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-mark'}
					></span>
				</span>
				<span class={agent ? 'text-phosphor' : 'text-ink-muted'}>Agent</span>
			</button>
		</nav>
	</header>
